 ...
 */
public class FacultyScheduleC implements Initializable {
            ...
            try {
                ...
                while (rs1.next()) {
                    ...
                    a[i][j]=a[i][j]+c;*/
                        ...
                        String sql = "SELECT * FROM dtt WHERE day='" + s1 + "' AND period='" + s2 + "' AND faculty='" + s + "' ORDER BY id;";
                        ResultSet rs = stmt.executeQuery(sql);
                        rs.last();
                        c = rs.getRow();
                        if (c > 0) {
                            //a[i][j]=rs.getString("section")+"\n"+rs.getString("room");
                            switch (j) {
                                case 1:
